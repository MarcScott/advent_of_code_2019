...
wiring_1_list = wire_1_description.split(",")
wiring_2_list = wire_2_description.split(",")

wire_1_nodes = [(0,0)]
wire_2_nodes = [(0,0)]

def build_nodes(wire_list, nodes):
    for i in wire_list:
        if i[0] == "R":
            for j in range(int(i[1:])):
                nodes.append((nodes[-1][0] + 1, nodes[-1][1]))
        if i[0] == "L":
            for j in range(int(i[1:])):
                nodes.append((nodes[-1][0] - 1, nodes[-1][1]))
        if i[0] == "U":
            for j in range(int(i[1:])):
                nodes.append((nodes[-1][0], nodes[-1][1] - 1))
        if i[0] == "D":
            for j in range(int(i[1:])):
                nodes.append((nodes[-1][0], nodes[-1][1] + 1))

build_nodes(wiring_1_list, wire_1_nodes)
build_nodes(wiring_2_list, wire_2_nodes)

intersections = set(wire_1_nodes) & set(wire_2_nodes)
man_distances = []
for node in intersections:
    man_distances.append(abs(node[0]) + abs(node[1]))
print(sorted(man_distances)[1])

true_distances = []
for node in intersections:
    true_distance = wire_1_nodes.index(node) + wire_2_nodes.index(node)
    true_distances.append(true_distance)
print(sorted(true_distances)[1])
